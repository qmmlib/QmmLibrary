...
void TRON::tron(double *w, const parameter *param, const struct model* model_, clock_t cpu_begin)
{
	...
	if (param->chat_level > 0)
	{
		...
		iter = 0;
		double objective_value = fun_obj->fun(w);
		__QMMCHAT__
	...
	while (iter <= max_iter && search)
	{
		...
		if (actred > eta0*prered)
		{
			...
			if(param->save_each_iter!=0) {char filenametosave[1024]; sprintf(filenametosave,"%s%d.model",param->save_each_iter,iter-1); save_model(filenametosave,model_);}
			if (param->chat_level > 0) {
				cpu_now = clock();
				cpu_time_elapsed = ((double) (cpu_now - cpu_begin)) / ((double) CLOCKS_PER_SEC);
				double objective_value = f;
				iter--;
				__QMMCHAT__
				iter++;
				cpu_begin += (clock() - cpu_now);
